...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
    ...
    if (!AccountMgr::IsGMAccount(GetSecurity()) && sObjectMgr->IsReservedName(name))
    {
    ...
    if (!AccountMgr::IsGMAccount(GetSecurity()) && class_ == CLASS_DEATH_KNIGHT && req_level_for_heroic > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
    {
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recv_data)
{
    ...
    // check name limitations
    if (!AccountMgr::IsGMAccount(GetSecurity()) && sObjectMgr->IsReservedName(newName))
    {
        WorldPacket data(SMSG_CHAR_RENAME, 1);
        data << uint8(CHAR_NAME_RESERVED);
        SendPacket(&data);
