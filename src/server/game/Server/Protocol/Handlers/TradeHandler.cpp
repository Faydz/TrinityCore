    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            if (myItems[i])
            {
                ...
                if (AccountMgr::IsGMAccount(_player->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
            ...
            if (hisItems[i])
            {
                ...
                if (AccountMgr::IsGMAccount(trader->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (his_trade->IsAccepted())
    {
        ...
        {
            if (AccountMgr::IsGMAccount(_player->GetSession()->GetSecurity()) && my_trade->GetMoney() > 0)
            {
            ...
            if (AccountMgr::IsGMAccount(trader->GetSession()->GetSecurity()) && his_trade->GetMoney() > 0)
            {
