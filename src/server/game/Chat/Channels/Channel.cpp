...
void Channel::Join(uint64 p, const char *pass)
{
    ...
    if (player)
    {
        if (HasFlag(CHANNEL_FLAG_LFG) &&
            sWorld->getBoolConfig(CONFIG_RESTRICTED_LFG_CHANNEL) && !AccountMgr::IsGMAccount(player->GetSession()->GetSecurity()) && player->GetGroup())
        {
            MakeNotInLfg(&data);
            SendToOne(&data, p);
            return;
...
void Channel::List(Player* player)
{
    ...
    else
    {
        ...
        for (PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
        {
            ...
            if (member && (AccountMgr::IsGMAccount(player->GetSession()->GetSecurity()) || member->GetSession()->GetSecurity() <= AccountTypes(gmLevelInWhoList)) &&
