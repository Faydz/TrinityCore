...
void Channel::Join(uint64 p, const char *pass)
{
    ...
    if (plr)
    {
        if (HasFlag(CHANNEL_FLAG_LFG) &&
            sWorld->getBoolConfig(CONFIG_RESTRICTED_LFG_CHANNEL) && !AccountMgr::IsGMAccount(plr->GetSession()->GetSecurity()) && plr->GetGroup())
        {
            MakeNotInLfg(&data);
            SendToOne(&data, p);
            return;
...
void Channel::List(Player* player)
{
    ...
    else
    {
        ...
        for (PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
        {
            ...
            if (plr && (!AccountMgr::IsGMAccount(player->GetSession()->GetSecurity()) || plr->GetSession()->GetSecurity() <= AccountTypes(gmLevelInWhoList)) &&
