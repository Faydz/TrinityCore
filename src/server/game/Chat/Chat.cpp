...
bool ChatHandler::HasLowerSecurityAccount(WorldSession* target, uint32 target_account, bool strong)
{
    ...
    if (!AccountMgr::IsModeratorAccount(m_session->GetSecurity()) && !strong && !sWorld->getBoolConfig(CONFIG_GM_LOWER_SECURITY))
        return false;
...
int ChatHandler::ParseCommands(const char* text)
{
    ...
    {
        if (m_session && !AccountMgr::IsGMAccount(m_session->GetSecurity()))
            return 0;
