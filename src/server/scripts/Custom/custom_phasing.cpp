   ...
   public:
	  ...
	  static bool HandlePhaseCreateCommand(ChatHandler * chat, const char * args)
	  {
		  ...
		  if(check)
		  {
			  do
			  {
				  ...
				  {
					  chat->SendSysMessage("|cffFF0000Ihr habt bereits eine Phase!|r");
					  return false;
				  }
				  else
				  {
		  ...
		  if(get_phase)
		  {
			  do
			  {
				  ...
				  {
					  chat->SendSysMessage("|cffFF0000Diese Phase ist bereits vergeben!|r");
					  return false;
				  }
				  else
				  {
	  ...
	  static bool HandlePhaseJoinCommand(ChatHandler * chat, const char * args)
	  {
		  ...
		  else if (phase == 1)
		  {
			  ...
			  chat->PSendSysMessage("|cff4169E1Ihr betretet nun Phase 1.|r (Default Phase)");
		  }
		  ...
		  if(hasPhase)
		  {
			 do
			 {
				 ...
				 {
					  chat->PSendSysMessage("|cffFF0000Ihr m\303\274sst zuerst eine Phase erstellen, bevor ihr einer beitreten k\303\266nnt!|r", phase);
					  return false;
				 }
		  ...
		  if(myPhase)
		  {
			  do
			  {
				  ...
				  {
					  chat->PSendSysMessage("|cffFF0000Ihr seid bereits in Phase %u!|r", phase);
					  return false;
				  }
		  ...
		  if(isCompleted)
		  {
			  do
			  {
				  ...
				  if(fields[0].GetUInt16() == 1) // if the phase is completed
				  {
				    ...
				    chat->PSendSysMessage("|cff4169E1Ihr betretet nun Phase %u.|r", phase);
				  }
				  else if(player->GetGUID() == fields[1].GetUInt64() && fields[2].GetUInt32() == phase)
				  {
				    player->SetPhaseMask(phase, true);
				    CharacterDatabase.PExecute("UPDATE phase SET get_phase='%u' WHERE guid='%u'", phase, player->GetGUID());
				    chat->PSendSysMessage("|cffffffffIhr seid nun in eurer eigenen Phase %u.|r", phase);
				  }
				  else // if the phase isn't completed
				  {
				    chat->PSendSysMessage("|cffFF0000Diese Phase ist noch nicht vervollst\303\244ndigt!|r \n Phase: %u", phase);
				    return false;
				  }
	  ...
	  static bool HandlePhaseDeleteCommand(ChatHandler * chat, const char * args)
	  {
		  ...
		  chat->SendSysMessage("|cffFFFF00Ihre Phase wurde gel\303\266scht.|r");
		  return true;
	  };
		  ...
		  if(phase)
		  {
			  do
			  {
				  ...
				  if(mfields[0].GetInt32() == fields[0].GetInt32())
				  {
					  ...
					  snprintf(msg, 255, "|cffFF0000Ihr wurdet aus der Phase %u gekickt. Von: %s!|r", fields[0].GetInt32(), player->GetName());
				  ...
				  {
					 snprintf(msg, 255, "|cffFF0000%s ist zurzeit nicht in ihrer Phase.|r", args);
					 chat->SendSysMessage(msg);
					 return false;
				  }
			  }while(phase->NextRow());
	  ...
	  static bool HandlePhaseGetCommand(ChatHandler * chat, const char * args)
	  {
		  ...
		  if(getPhaseAndOwnedPhase)
		  {
			  do
			  {
				  ...
				  chat->PSendSysMessage("|cffADD8E6Current Phase: %u \n Phase in Besitz: %u|r", fields[1].GetUInt32(), fields[2].GetUInt32());
	  ...
	  static bool HandlePhaseCompleteCommand(ChatHandler * chat, const char * args)
	  {
		  ...
		  if(isOwnerOfAPhase)
		  {
			  do
			  {
				  ...
				  if(fields[0].GetInt32() == 0)
				  {
					  chat->SendSysMessage("|cffFF0000Konnte Phase nicht vervollst\303\244ndigen.|r");
					  return false;
				  }
		  ...
		  if(yesorno == 1)
		  {
		      chat->SendSysMessage("|cffFFA500Ihr habt eure Phase nun vervollst\303\244ndigt. Sie ist nun \303\266ffentlich.|r");
	  ...
	  static bool HandlePhaseAddMemberCommand(ChatHandler * chat, const char * args)
	  {
		  ...
		  if (!target)
		  {
			 chat->SendSysMessage("Kein Charakter ausgew\303\244hlt.");
		  ...
		  if(target == player)
			...
			if(isOwnerOfAPhase)
			{
				do
				{
					...
					if(fields[0].GetInt32() == 0)
					{
						chat->PSendSysMessage("|cffFF0000 %s hat keine eigene Phase. Deshalb kann er nicht zu ihrer Phase hinzugef\303\274gt werden.|r", target->GetName());
						return false;
					}
				}while(isOwnerOfAPhase->NextRow());
			}
			...
			if(isMember)
			{
				do
				{
					...
					{
						chat->PSendSysMessage("|cffFF0000 %s ist bereits in ihrer Phase|r", target->GetName());
						return false;
					}
			...
			if(isOwner)
			{
				do
				{
					...
					{
						chat->PSendSysMessage("|cffFF0000Ihr k\303\266nnt nicht euch selbst oder jemand anderen zu einer Phase eines anderen hinzuf\303\274gen.|r");
						return false;
					}
				}while(isOwner->NextRow());
			}
			chat->PSendSysMessage("|cffFFA500You %s wurde erfolgreich zu ihrer Phase %u hinzugef\303\274gt.|r", target->GetName(), ph);
	  ...
	  static bool HandlePhaseGoCommand(ChatHandler * handler, const char * args)
	  {
        ...
        if (!objectInfo)
        {
            handler->PSendSysMessage("Gameobjekt existiert nicht.", objectId);
        ...
        if (objectInfo->displayId && !sGameObjectDisplayInfoStore.LookupEntry(objectInfo->displayId))
        {
            ...
            handler->PSendSysMessage("Das Gameobjekt hat ung\303\274ltige Daten", objectId);
            handler->SetSentErrorMessage(true);
            return false;
        }
		...
		if (phasing)
		{
			...
			if(res)
			{
				do
				{
					...
					if(val != value)
					{
						handler->SendSysMessage("Ihr seid nicht in dieser Phase!");
						return false;
					}
					...
					if(result)
					{
						do
						{
							...
							if(fields[0].GetInt32() == 0)
							{
								handler->SendSysMessage("Ihr m\303\274sst zuerst zu dieser Phase hinzugef\303\274gt werden, bevor ihr bauen k\303\266nnt.");
								return false;
							}
        ...
        handler->PSendSysMessage("Gameobjekt wurde hinzugef\303\274gt.", objectId, objectInfo->name.c_str(), guidLow, x, y, z);
        return true;
    }
	  ...
	  static bool HandlePhaseGoTargetCommand(ChatHandler * handler, const char * args)
	  {
					...
					do
					{
						...
						if(result)
						{
							do
							{
								...
								if(fields[0].GetInt32() == 0)
								{
									handler->SendSysMessage("Ihr m\303\274sst zuerst zu dieser Phase hinzugef\303\274gt werden, bevor ihr ein Gameobject anvisieren k\303\266nnt.");
									return false;
								}
							}while(result->NextRow());
						}
        ...
        else
        {
          ...
          if (!result)
          {
             handler->SendSysMessage("Das Objekt wurde nicht gefunden!");
             return true;
          }
          ...
          if (!found)
          {
             handler->PSendSysMessage("Gameobjekt existiert nicht.", id);
             return false;
          } 
          ...
          handler->PSendSysMessage("Details: ", lowguid, goI->name.c_str(), lowguid, id, x, y, z, mapid, o, phase);
          ...
          if (target)
          {
             ...
             handler->PSendSysMessage("", defRespawnDelayStr.c_str(), curRespawnDelayStr.c_str());
		  }
        return true;
	  };
	  ...
	  static bool HandlePhaseGoDeleteCommand(ChatHandler * handler, const char * args)
	  {
		  ...
		  if (!obj)
		  {
			  handler->PSendSysMessage("Gameobjekt wurde nicht gefunden.", lowguid);
			  handler->SetSentErrorMessage(true);
			  return false;
		  }
					...
					do
					{
						...
						if(result)
						{
							do
							{
								...
								if(fields[0].GetInt32() == 0)
								{
									handler->SendSysMessage("Ihr m\303\274sst zuerst zu dieser Phase hinzugef\303\274gt werden, bevor Ihr ein Objekt l\303\266schen k\303\266nnt");
									return false;
								}
							}while(result->NextRow());
						}
		  ...
		  if (owner_guid)
		  {
			  ...
			  {
				  handler->PSendSysMessage("", GUID_LOPART(owner_guid), obj->GetGUIDLow());
				  handler->SetSentErrorMessage(true);
				  return false;
			  }
		  ...
		  obj->Delete();
		  obj->DeleteFromDB();
		  handler->PSendSysMessage("Gameobjekt wurde gel\303\266scht.", obj->GetGUIDLow());
		  return true;
	  };
#endif
	...
	static bool HandlePhaseDeleteNpcCommand(ChatHandler * handler, const char * args)
	{
					...
					do
					{
						...
						if(result)
						{
							do
							{
								...
								if(fields[0].GetInt32() == 0)
								{
									handler->SendSysMessage("Ihr m\303\274sst als erstes zu dieser Phase hinzugef\303\274gt werden, bevor Ihr ein Gameobject hinzuf\303\274gen k\303\266nnt.");
									return false;
								}
							}while(result->NextRow());
						}
        ...
        {
            handler->SendSysMessage("Ihr m\303\274sst zuerst eine Kreatur ausw\303\244hlen.");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        handler->SendSysMessage("NPC wurde gel\303\266scht.");
